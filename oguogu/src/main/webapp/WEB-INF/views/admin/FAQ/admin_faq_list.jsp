...
<script type="text/javascript">
	...
	function search() {
	    ...
	    if (keyword === "") {
	        ...
	        // 검색어를 포함한 URL 생성
	        var searchUrl = "/faq_search.do?keyword=" + encodeURIComponent(keyword);
	        // 생성한 URL로 이동
  ...
  <tbody>
   <c:choose>
	<c:when test="${empty f_list}">
					...
					<c:otherwise>
						<c:forEach var="ad" items="${f_list}"  varStatus="vs">
							<tr>
							<c:if test="${ad.status == 0}">
								<td>${ad.faq_idx}</td>
								<td><a id="atitle" href="/ad_faq_onelist.do?faq_idx=${ad.faq_idx}&cPage=${paging.nowPage}">${ad.faq_title}</a></td>
								<td>${ad.admin_nickname }</td>
								<td>${ad.faq_date.substring(0,10)}</td>
								 <c:choose>
                    <c:when test="${result.faq_ing == 0}">
                        <td>게시</td>
                    </c:when>
                    <c:otherwise>
                        <td>미게시</td>
 ...
 <tfoot>
   <!-- 페이징  기법 -->
		...
		<td colspan="7">
			<ol class="paging"> 
				...
				<c:choose>
					...
					<c:otherwise>
						<li><a
							href="ad_faq_list.do?cPage=${paging.beginBlock-paging.pagePerBlock }">이전으로</a></li>
					</c:otherwise>
				</c:choose>
				<!-- 페이지번호들 -->
				<c:forEach begin="${paging.beginBlock }" end="${paging.endBlock }" step="1" var="k">
					...
					<c:if test="${ k != paging.nowPage}">
						<li><a href="/ad_faq_list.docPage=${k}">${k}</a></li>
					</c:if>
				</c:forEach>
				...
				<c:choose>
					...
					<c:otherwise>
						<li><a
							href="/ad_sup_faq.do?cPage=${paging.beginBlock+paging.pagePerBlock }">>></a></li>
					</c:otherwise>
				</c:choose>
			</ol>
		</td>
		<td>
		<input type="button" class="button" value="FAQ등록" onclick=" write_go()">
		</td>
</tfoot>
</table>
	</div>
	...
	<div id='liSearchOption' style="margin: auto;">
            	...
            	<form name="search" action="/faq_search.do" method="get" onsubmit="return validateSearch()">
                <div>
                    <select name="searchtype">
                        <option value="faq_title">제목</option>
                         <option value="faq_date">날짜</option>
                        <option value="admin_nickname">작성자</option>                        
                    </select>
