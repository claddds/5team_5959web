...
<c:choose>
    ...
    <c:otherwise>
        ...
        <c:forEach var="result" items="${search}">
            <tr>
                <td>${result.faq_idx}</td>
				<td><a id="atitle" href="/ad_faq_onelist.do?faq_idx=${result.faq_idx}&cPage=${paging.nowPage}">${result.faq_title}</a></td>
				<td>${result.admin_nickname }</td>
				<td>${result.faq_date.substring(0,10)}</td>
                <c:choose>
...
<tfoot><!-- 페이징  기법 -->
	<tr>
		<td colspan="6">
			<ol class="paging"> 
				...
				<c:choose>
					...
					<c:otherwise>
						<li><a
							href="/faq_search.do?cPage=${paging.beginBlock-paging.pagePerBlock }"><<</a></li>
					</c:otherwise>
				</c:choose>
				<!-- 페이지번호들 -->
				<c:forEach begin="${paging.beginBlock }" end="${paging.endBlock }" step="1" var="k">
					...
					<c:if test="${ k != paging.nowPage}">
						<li><a href="/faq_search.do?cPage=${k}">${k}</a></li>
					</c:if>
				</c:forEach>
        ...
        <td>
            <input type="button" value="FAQ등록" class="button" onclick=" write_go()">
        </td>
    </tr>
</tfoot>
</table>
