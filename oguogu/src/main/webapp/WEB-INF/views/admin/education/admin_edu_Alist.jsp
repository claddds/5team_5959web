  ...
  <tbody>
   <c:choose>
	<c:when test="${empty e_alist}">
					...
					<c:otherwise>
						<c:forEach var="e" items="${e_alist}"  varStatus="vs">
							<tr>
		...
		<td colspan="6">
			<ol class="paging"> 
				...
				<c:choose>
					...
					<c:otherwise>
						<li><a
							href="/edu_Alist.do?cPage=${paging.beginBlock-paging.pagePerBlock }">이전으로</a></li>
					</c:otherwise>
				</c:choose>
				<!-- 페이지번호들 -->
				<c:forEach begin="${paging.beginBlock }" end="${paging.endBlock }" step="1" var="k">
					...
					<c:if test="${ k != paging.nowPage}">
						<li><a href="/edu_Alist.do?cPage=${k}">${k}</a></li>
					</c:if>
				</c:forEach>
				...
				<c:choose>
					...
					<c:otherwise>
						<li><a
							href="/edu_Alist.do?cPage=${paging.beginBlock+paging.pagePerBlock }">다음으로</a></li>
					</c:otherwise>
				</c:choose>
			</ol>
		</td>
