...
console.log("현재 페이지: " + ${paging.nowPage});
	...
	function search() {
	    ...
	    if (keyword === "") {
	        ...
	        // 검색어를 포함한 URL 생성
	        var searchUrl = "/edu_Asearch.do?keyword=" + encodeURIComponent(keyword);
	        // 생성한 URL로 이동
...
<c:choose>
    ...
    <c:otherwise>
        ...
        <c:forEach var="result" items="${search}">
            <tr>
                <td>${result.edu_idx}</td>
                <td>${result.edu_kind}</td>
                <td>${result.edu_aml_kind}</td>
                <td><a id="atitle" href="/edu_onelist.do?edu_idx=${result.edu_idx}&cPage=${paging.nowPage}">${result.edu_title}</a></td>
                <td>${result.admin_nikname}</td>
                <td>${result.edu_date.substring(0,10)}</td>
                <c:choose>
                    <c:when test="${result.edu_open == 0}">
                        <td>게시</td>
                    </c:when>
                    <c:otherwise>
                        <td>미게시</td>
	...
	<tr>
		  <td colspan="7">
            ...
            <c:choose>
                  ...
                  <c:otherwise>
                     <!-- a링크처리 -->
                     <li><a href="/edu_Asearch.do?cPage=${paging.beginBlock-paging.pagePerBlock}">이전으로</a></li>
                  </c:otherwise>
            </c:choose>
             <!-- 블록안에 들어간 페이지번호들 -->
            <c:forEach begin="${paging.beginBlock}" end="${paging.endBlock }" step="1" var="k">
               ...
               <c:if test="${k != paging.nowPage}">
                     <!-- a링크처리 -->
                     <li><a href="/edu_Asearch.do?cPage=${k}">${k}</a></li>
               </c:if>
            </c:forEach>
            <!-- 다음 -->
              <c:choose>
                  ...
                  <c:otherwise>
                     <!-- a링크처리 -->
                     <li><a href="/edu_Asearch.do?cPage=${paging.beginBlock+paging.pagePerBlock}">다음으로</a></li>
                  </c:otherwise>
            </c:choose> 
         </ol>
        </td>
        <td>
            <input type="button" value="교육등록" class="button" onclick=" write_go()">
        </td>
    </tr>
</tfoot>
</table>
...
<!--검색 항목-->
	<div id='liSearchOption' style="margin: auto;">
            	<form name="search" action="/edu_Asearch.do" method="get" onsubmit="return validateSearch()">
                <div>
