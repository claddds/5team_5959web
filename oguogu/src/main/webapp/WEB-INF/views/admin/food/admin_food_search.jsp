...
<c:choose>
    ...
    <c:otherwise>
        ...
        <c:forEach var="fd" items="${search}"  varStatus="vs">
							<tr>
...
<tfoot><!-- 페이징  기법 -->
	<tr>
		<td colspan="6">
			<ol class="paging"> 
				...
				<c:choose>
					...
					<c:otherwise>
						<li><a
							href="/food_search.do?cPage=${paging.beginBlock-paging.pagePerBlock }"><<</a></li>
					</c:otherwise>
				</c:choose>
				<!-- 페이지번호들 -->
				<c:forEach begin="${paging.beginBlock }" end="${paging.endBlock }" step="1" var="k">
					...
					<c:if test="${ k != paging.nowPage}">
						<li><a href="/food_search.do?cPage=${k}">${k}</a></li>
					</c:if>
				</c:forEach>
        ...
        <td>
            <input type="button" value="상품등록" class="button" onclick=" write_go()">
        </td>
    </tr>
</tfoot>
</table>
