...
<!-- 사이드바 구역 -->
 <div class="sidebar" >
  ...
  <tbody>
   <c:choose>
	<c:when test="${empty ad_slist}">
						<tr>
							<td colspan="5"><h2>자료가 존재하지 않습니다.</h2></td>
						</tr>
					</c:when>
					<c:otherwise>
						<c:forEach var="ad" items="${ad_slist}"  varStatus="vs">
							<tr>
		...
		<td colspan="7">
			<ol class="paging"> 
				...
				<c:choose>
					...
					<c:otherwise>
						<li><a
							href="ad_sup_list.do?cPage=${paging.beginBlock-paging.pagePerBlock }">이전으로</a></li>
					</c:otherwise>
				</c:choose>
				<!-- 페이지번호들 -->
				<c:forEach begin="${paging.beginBlock }" end="${paging.endBlock }" step="1" var="k">
					...
					<c:if test="${ k != paging.nowPage}">
						<li><a href="/ad_sup_list.docPage=${k}">${k}</a></li>
					</c:if>
				</c:forEach>
				...
				<c:choose>
					...
					<c:otherwise>
						<li><a
							href="/ad_sup_list.do?cPage=${paging.beginBlock+paging.pagePerBlock }">>></a></li>
					</c:otherwise>
				</c:choose>
			</ol>
		</td>
		<td>
		<input type="button" class="button" value="공지등록" onclick=" write_go()">
		</td>
	</tr>
</tfoot>
</table>
