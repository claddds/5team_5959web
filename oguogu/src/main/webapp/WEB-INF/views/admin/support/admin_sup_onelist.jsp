...
<script type="text/javascript">
	function edit_go(f) {
		f.action="/sup_UpdateForm.do";
		f.submit();
	}
	function delete_go(f){
	 if (confirm("공지사항을 삭제하시겠습니까?")) {
		f.action="/sup_delete.do";
		f.submit();
	}
	}
</script>
		...
		<p align="center"><a id="link" href="/ad_sup_list.do">공지사항 전체보기</a></p>
		 <!-- 파일 첨부하려면  -->
			...
			<tr align="center">
					...
					<td align="left" style="border-radius:20px; border: 3px solid orange; ">${sup_vo.not_title }</td>
				</tr>
				<tr align="center">
					<td bgcolor="FFA629" width="150px" style="background-color: #FFA629; border: 3px solid white; color: white;font-weight: bold;">작성자</td>
					<td align="left" style="border-radius:20px; border: 3px solid orange; ">${sup_vo.admin_nickname }</td>
				</tr>
				<tr align="center">
					<td bgcolor="FFA629" width="150px" style="background-color: #FFA629; border: 3px solid white; color: white;font-weight: bold;">내용</td>
					<td align="left" style="border-radius:20px; border: 3px solid orange; "> ${sup_vo.not_content }</td>
				</tr>			
				...
				<tr align="center">
					...
					<c:choose>
						<c:when test="${empty sup_vo.not_fname }">
							<td align="left" style="border-radius:20px; border: 3px solid orange; "><b></b></td>
						</c:when>
						<c:otherwise>
							<td>
								<a href="/guestbook2_down.do?f_name=${sup_vo.not_fname}">
									<img src="resources/images/${sup_vo.not_fname}" style="width: 150px;">
								</a> 	
							</td>
						</c:otherwise>
					</c:choose>
				...
				<tfoot>
					<tr align="center">
						<td colspan="2" style="margin: auto; border-radius:20px; border: 3px solid orange; ">
							<input type="hidden" name="not_idx" value="${sup_vo.not_idx}">
							<input type="button" class="button" value="수정" onclick="edit_go(this.form)" />
							&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
							<input type="hidden" name="not_title" value="${sup_vo.not_title}">
