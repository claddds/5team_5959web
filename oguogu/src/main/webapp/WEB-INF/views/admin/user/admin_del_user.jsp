...
<title>탈퇴회원관리 (관리자 화면)</title>
<script
	src="https://ajax.googleapis.com/ajax/libs/jquery/3.6.0/jquery.min.js"></script>
<style type="text/css">
#admin_del_user_title {
	margin: 10px auto;
...
<body style="width: 100%;">
	...
	<div style="border-bottom: 1px solid #bcc;">
		<p id="admin_del_user_title">탈퇴 회원 관리</p>
	</div>
	<div id="admin_sea">
	...
	<div class="admin_deluser_list" style="display: flex;">
		<table class="table">
			<colgroup>
				<col width="140px" />
				<col width="140px" />
		...
		function getUserInfo(param) {
			$.ajax({
						...
						success : function(res) {
							var html = '';
							if (res.dellist.length > 0) {
								for (var i = 0; i < res.dellist.length; i++) {
									html += '<tr>';
									html += '<td>' + res.dellist[i].user_id
											+ '</td>';
									html += '<td>' + res.dellist[i].nickname
											+ '</td>';
									html += '<td>' + res.dellist[i].email
											+ '</td>';
									html += '<td>' + res.dellist[i].type
											+ '</td>';
									html += '<td>' + res.dellist[i].initdate
											+ '</td>';
									html += '<td>' + res.dellist[i].enddate
											+ '</td>';
									html += '<td>' + res.dellist[i].status			
											+ '</td>';
									html += '</tr>';
								}
								// 페이징
								...
								html += '<tr><td colspan="7"><h2 style="text-align: center;">자료가 존재하지 않습니다.</h2></td></tr>';
							}
							$('.result').html(html);
							$('.paging').html(pageHtm);
