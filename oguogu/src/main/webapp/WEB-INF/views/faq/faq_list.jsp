		...
		<td colspan="5">
			<ol class="paging"> 
				...
				<c:choose>
					...
					<c:otherwise>
						<li><a
							href="/faq_list.do?cPage=${paging.beginBlock-paging.pagePerBlock }">이전으로</a></li>
					</c:otherwise>
				</c:choose>
				<!-- 페이지번호들 -->
				<c:forEach begin="${paging.beginBlock }"
					...
					<c:if test="${ k != paging.nowPage}">
						<li><a href="/faq_list.do??cPage=${k}">${k}</a></li>
					</c:if>
				</c:forEach>
				...
				<c:choose>
					...
					<c:otherwise>
						<li><a
							href="/faq_list.do?cPage=${paging.beginBlock+paging.pagePerBlock }">다음으로</a></li>
					</c:otherwise>
				</c:choose>
			</ol>
		</td>
